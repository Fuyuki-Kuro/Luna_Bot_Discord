...
A seguir, a estrutura de diretórios e arquivos do bot:

Claro! Entendi que você quer um README.md que mostre a estrutura do seu projeto Wild Rift Nexus Bot de forma clara.

Aqui está o conteúdo do README.md com a estrutura que você me forneceu:
...
wild_rift_bot/
│
├── .env                 # Variáveis de ambiente (TOKEN do Discord, URI do MongoDB).
# Contém informações sensíveis que não devem ser versionadas.
├── .gitignore           # Lista de arquivos e diretórios que o Git deve ignorar (ex: .env, pycache, venv/).
├── requirements.txt     # Lista todas as bibliotecas Python necessárias para o projeto.
# Pode ser instalada via pip install -r requirements.txt.
│
├── main.py              # Ponto de entrada principal do bot.
# Responsável por inicializar o cliente do Discord, conectar ao MongoDB e carregar os cogs.
│
├── cogs/                # Diretório que contém os módulos (cogs) do bot.
│   │                    # Cada cog agrupa comandos e eventos relacionados a uma funcionalidade específica.
│   ├── init.py      # Torna 'cogs' um pacote Python.
│   ├── admin.py         # Contém comandos para administração do servidor (gerenciamento de cargos, configurações do bot, etc.).
│   ├── teams.py         # Implementa o CRUD (Criação, Leitura, Atualização, Deleção) de equipes.
│                        # Gerencia a criação, entrada, saída e visualização de equipes de jogadores.
│   ├── guilds.py        # Implementa o CRUD de guildas (clãs) do jogo.
│                        # Gerencia a criação e visualização de estruturas de guilda.
│   ├── tournaments.py   # Contém comandos para a criação, gerenciamento e participação em torneios.
│                        # Inclui funcionalidades como inscrição, início e reporte de resultados.
│   └── wild_rift.py     # Abriga comandos específicos relacionados ao Wild Rift (ex: busca de estatísticas de jogadores, notícias do jogo, se houver integração com API).
│
├── database/            # Módulo para interação com o banco de dados MongoDB.
│   │
│   ├── init.py      # Torna 'database' um pacote Python.
│   ├── connection.py    # Gerencia a conexão e desconexão com o servidor MongoDB.
│   └── models.py        # Define os modelos de dados Python que representam as estruturas das coleções no MongoDB (equipes, guildas, torneios).
│
└── utils/               # Diretório para funções utilitárias e auxiliares.
│
├── init.py      # Torna 'utils' um pacote Python.
├── constants.py     # Armazena constantes do projeto (ex: IDs de canais, IDs de cargos, strings de mensagens padrão).
├── embeds.py        # Contém funções para criar embeds padronizados, melhorando a apresentação visual das mensagens do bot.
└── permissions.py   # Funções auxiliares para checagem de permissões de usuários e cargos específicos.
